...
description: マルウェア感染・サイト改ざん・データ漏洩・設定事故・パフォーマンス障害など、証跡を残しながら進める必要がある調査（防御目的のインシデントレスポンス）で使う。侵入時刻の特定、タイムスタンプの信頼性評価（access log / btime / ctime / mtime / atime の信頼度階層）、touch 偽装の検出、報告書での時刻の書き方を扱う。「インシデント調査」「侵入時刻」「タイムスタンプ偽装」「フォレンジック」「改ざん調査」「マルウェア調査」等の文脈で必ず参照する。
---
