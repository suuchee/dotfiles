...
作業開始時点で既にステージング済みの変更がある場合、ユーザーまたは別セッションが意図的にステージした可能性があるため、勝手に `git restore --staged` 等で解除しない。

そのまま残し、コミット時は path 指定でコミットする：

```sh
git commit <path>...   # 指定 path の変更のみコミット（既存 staged は据え置き）
```

`git commit`（path 指定なし）は staged 全体を取り込むため、既存 staged を巻き込んでしまう。意図しない混入を避けるため、必ず path を明示する。

ステージされている内容が自分の作業と紐付かない・由来が不明な場合は、コミットに含めるかどうかをユーザーに確認する。
...
粒度設計の原則（1 ブランチ = 1 単位）と例外（複数単位が混ざる場合のコミット統合）、および「戻しやすさを目的とする」理由は `~/.claude/rules/git-revert-granularity.md` を参照。
