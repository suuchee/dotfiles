...
粒度は **片方だけ revert したときに整合性が壊れるかどうか** で判断する。壊れる関係にある変更は同一コミットに含める。

ここでいう「revert」は **戻したい単位**（コミット / ブランチ / PR）を意味する。ブランチ全体が 1 つのリファクタリング操作（例: 切り出し・rename）のみで構成され、ブランチ / PR 単位で revert される前提なら、ブランチ内のコミット粒度はレビュー・履歴の見通しに合わせて分割してよい（下記の「1コミットにまとめるべき操作」の例外）。
...
- revert 単位が異なる（機能追加と無関係なリファクタリング 等）
- レビュー観点が異なる（アプリコード / インフラ設定 等）
- マージ・リリースタイミングが異なる
