...
> `<path>` に今回のタスク外の変更が同一ファイル内に混在しています（hunk 数: N）。`git add -p` / `git restore --staged -p` で切り分けるか、このファイルを今回のコミットから外すか指示ください。

> `<path>` は index と worktree で別状態（`MM` / `AM`）です。hunk の非対話切り分けが危険なため、対話的 `-p` をお願いするか、今回のコミットから外すか指示ください。
