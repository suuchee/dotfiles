# ADR 置き換え（Supersede）フロー

既存ADRを新しいADRで置き換える対話フロー。公式 MADR の `status: "superseded by ADR-NNNN"` 形式に従う。
...
## Step 0: supersede が必要かの判定（最優先）

ADRは原則 immutable。`accepted` 後の変更は次の3レーンに振り分ける。**まずこの判定を必ず通すこと。**
...
決定の妥当性を補強する事後観測のみ。本フローは使わず、ADRの `Consequences` セクションに `#### Updates` サブセクションを設けて1行追記（frontmatter の `date` も更新）して終了：

- 想定通りの効果が観測された（「p99 が想定通り 30% 改善」「Bad で挙げた懸念は顕在化しなかった」）
- 別方面の検証でも採用案が優位だった
- 採用理由の補強事実（元の理由を否定せず裏付けるデータ）

→ supersede しない。`adr-conventions.md` の「受理済みADRへの事実追記」に従う。
...
判断に迷う場合は supersede を選ぶ（保守的な側へ倒す）。詳細は `adr-conventions.md` の「受理済みADRの変更ルール」を参照。

## Step 1: 置き換え対象の特定

ユーザー指定または対話で特定された旧ADRを確認する。

対象が不明な場合：

1. 既存ADRの一覧を表示する（`list-flow.md` のフローに従う）
2. ユーザーに置き換え対象を選択してもらう（`AskUserQuestion`）

## Step 2: 旧ADRの内容確認

旧ADRを読み取り、以下を確認する：

- 現在のステータスが `accepted` であること（`proposed` の場合は新ADRを作らず直接編集を提案）
- 決定内容とその背景
- 何が入れ替わるのか（採用案 / 採用理由 / Y-Statement の構成要素）を Step 0 の判定と整合させて確定

## Step 3: 新ADRの作成

`create-flow.md` のフローに従って新しいADRを作成する。ただし以下を追加：

- **Context and Problem Statement** に旧ADRの決定がなぜ変更されるかを記述（採用案・採用理由のどこが入れ替わったか）
...
NNNN は新ADRの4桁ゼロ埋め番号。**本文は変更しない**（immutable 原則）。
