...
# PR 作成（指示がない限り自分をアサイン）
gh pr create --title "タイトル" --body "説明" --assignee "@me"
...
PR の body（説明）は **現在のブランチの成果物** を正として書く。

- 変更の経緯・変遷（リネーム履歴、段階的な再構成の過程など）は含めない
  - 経緯は git log を見ればわかるため、PR 説明に重複して書く必要はない
  - 明示的に「経緯も書いて」と指示があった場合のみ含める
- レビュアーが知りたいのは「今何があるか」「何が変わったか（develop との diff）」
